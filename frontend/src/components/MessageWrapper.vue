<template>
  <div class="wrapper">
    <br>
    <div v-for="(msg, index) in messageData" class="message"
         :style="doesFoldingDisplay(messageData[index - 1], msg) ? 'padding: 8px 10px 8px 10px; min-height: 0;' : ''">
      <img v-if="!doesFoldingDisplay(messageData[index - 1], msg)"
           class="player-avatar"
           :src="`https://crafatar.com/avatars/${msg.playerUUID}?size=50`" alt="头像">
...
const doesFoldingDisplay = (preMsg, currMsg) => {
  if (preMsg && currMsg) {
    // 同一个人且间隔小于5分钟
    return preMsg.username === currMsg.username && currMsg.timestamp - preMsg.timestamp < 5 * 60 * 1000
  }
  return false
}
